...
public class ArrayListSource<E> extends AbstractList<E> implements List<E>, RandomAccess, Cloneable, java.io.Serializable {
	...
	/**
	 * Save the state of the <tt>ArrayListSource</tt> instance to a stream (that is,
	 * serialize it).
	 *
	 * @serialData The length of the array backing the <tt>ArrayListSource</tt> instance
	...
	/**
	 * Reconstitute the <tt>ArrayListSource</tt> instance from a stream (that is,
	 * deserialize it).
	 */
	...
	private class SubList extends AbstractList<E> implements RandomAccess {
		...
		protected void removeRange(int fromIndex, int toIndex) {
			checkForComodification();
			super.removeRange(parentOffset + fromIndex, parentOffset + toIndex);
			this.modCount = super.modCount;
			this.size -= toIndex - fromIndex;
		}
		...
		public Spliterator<E> spliterator() {
			checkForComodification();
			return new ArrayListSourceSpliterator<E>(ArrayListSource.this, offset, offset + this.size, this.modCount);
		}
	}
	...
	@Override
	public Spliterator<E> spliterator() {
		return new ArrayListSourceSpliterator<>(this, 0, -1, 0);
	}

	/** Index-based split-by-two, lazily initialized Spliterator */
	static final class ArrayListSourceSpliterator<E> implements Spliterator<E> {

		/*
		 * If ArrayListSources were immutable, or structurally immutable (no adds, removes,
		...
		ArrayListSourceSpliterator(ArrayListSource<E> list, int origin, int fence, int expectedModCount) {
		...
		public ArrayListSourceSpliterator<E> trySplit() {
			int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
			return (lo >= mid) ? null : // divide range in half unless too small
					new ArrayListSourceSpliterator<E>(list, lo, index = mid, expectedModCount);
		}
